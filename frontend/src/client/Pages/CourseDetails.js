...
const CourseDetails = () => {
  ...
  return (
    <>
      {sessionStorage.getItem("coursePayed") === "true" && (
        <CourseAfterPayment course={course} />
      )}
      {sessionStorage.getItem("coursePayed") === "false" && (
        <CourseBeforePayment course={course} />
      )}

      <div className="m-auto card__slider__recommended pt-2">
        <div
          className="recommended__text d-flex justify-content-between m-auto mt-3"
          style={{ width: "90%" }}
        >
          <h4 className="p-2">
